...
```{r}
ggplot(df, aes(x = as.Date(Date), y = BSS)) +
  geom_line() +
...
ggplot(df_filtered, aes(x = as.Date(Date), y = BSS)) +
  geom_line() +
  geom_vline(xintercept = as.Date("2019-03-14"), linetype = "dashed", color = "red") +
    annotate("text", x = as.Date("2019-03-14"), y = 80, label = "80%", color = "red", vjust = -0.5) +
...
```{r}
# Filtrer les données entre 2010 et 2012
df_filtered <- df %>% 
...
ggplot(df_filtered, aes(x = as.Date(Date), y = BES)) +
  geom_line() +
  labs(title = "BES Evolution 2010-12-01 to 2011-08-31", x = "Date", y = "BES") +
  theme_minimal()
...
ggplot(df_filtered, aes(x = as.Date(Date), y = BES)) +
  geom_line() +
  labs(title = "BES Evolution 2019-12-01 to 2020-05-31", x = "Date", y = "BES") +
  theme_minimal()
```
